...
class SingleLabelMetrics(torchmetrics.Metric):
    ...
    def save(self, model, classifier_type):
        f = open(model + "_" + classifier_type + "_" + "test_metrics.txt", "w")
...
class BinarySegmentationMetrics(torchmetrics.Metric):
    def __init__(self,):
        super().__init__()

        self.add_state("true_positives", torch.zeros(1))
        self.add_state("false_positives", torch.zeros(1))
        self.add_state("true_negatives", torch.zeros(1))
        self.add_state("false_negatives", torch.zeros(1))
    ...
    def compute(self):
        ...
        return {'Precision': self.precision.item(), 'Recall': self.recall.item(), 'F-Score': self.f_score.item()}

    def save(self, model, classifier_type):
        f = open(model + "_" + classifier_type + "_" + "test_segmentation_metrics.txt", "w")
        f.write("Precision: " + str(self.precision.item()) + "\n")
        f.write("Recall: " + str(self.recall.item()) + "\n")
        f.write("F-Score: " + str(self.f_score.item()))
        f.close()
