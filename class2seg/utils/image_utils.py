...
def save_mask(mask, path_file):
    path_file.mkdir(exist_ok=True, parents=True)
    # path_file = os.path.splitext(filename)[0]

    img = mask.detach().cpu().numpy().squeeze()
    plt.imsave(path_file / "cam.jpg", img)
    plt.imsave(path_file / "cam_gray.jpg", img, cmap='gray')
    np.save(path_file / "cam_grayscale", img)
